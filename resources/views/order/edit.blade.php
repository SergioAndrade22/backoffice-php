...
<x-app-layout>
    ...
    <form class="flex pt-8 h-screen items-start justify-center overflow-auto" action="/orders/{{$order->id}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <optgroup label="Select a Table">
                            @foreach ($tables as $table)
                                <option class="uppercase" value="{{$table->id}}"
                                    {{($table->id == $order->table_id) ? 'selected' : ''}}>{{$table->id.": ".strtoupper($table->description)}}</option>
                            @endforeach
                        </optgroup>
                    </select>
                </div>
                ...
                <table id="item-container" class="table-auto text-center divide-y divide-gray-200">
                    ...
                    <tbody id="item-container-body">
                        @foreach($order->items as $item)
                            <tr id="{{explode(' ', $item->name)[0]}}">
                                <td colspan="2" class="px-4 py-4 whitespace-nowrap text-left"><button class="rounded-full leading-3 p-1 text-center bg-red-400" type="button" onclick="remove('{{explode(' ', $item->name)[0]}}')">-</button> {{$item->name}}</td>
                                <td class="px-4 py-4 whitespace-nowrap"><input class="text-right outline-none" name="items[{{$item->id}}][id]" value="{{$item->id}}" readonly/></td>
                                <td class="text-right px-4 py-4 whitespace-nowrap"><input class="text-right outline-none" id="{{explode(' ', $item->name)[0].'-amount'}}" name="items[{{$item->id}}][amount]" value="{{$item->pivot->amount}}" readonly/></td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex justify-between">
                                        <button class="rounded-full leading-3 p-1 text-center bg-gray-100 outline-none" type="button" onclick="substractAmount('{{explode(' ', $item->name)[0]}}', {{$item->cost}})">-</button>
                                        <button class="rounded-full leading-3 p-1 text-center bg-gray-100 outline-none" type="button" onclick="addAmount('{{explode(' ', $item->name)[0]}}', {{$item->cost}})">+</button>
                                    </div>
                                </td>
                                <td class="text-right px-4 py-4 whitespace-nowrap"><input class="text-right outline none" id="{{explode(' ', $item->name)[0].'-cost'}}" name="items[{{$item->id}}][cost]" value="{{$item->cost}}" readonly/></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
